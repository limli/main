...
****
• `INGREDIENT_NAME` and `INGREDIENT_UNIT` must contain only alphabets and spaces, and must contain at least 2 alphabets. (i.e, `in/c` is not accepted while `in/ch` is).
...
•`RECIPE_NAME` must contain only alphabets and spaces, and must contain at least 2 alphabets. (i.e, `r/c` is not accepted while `r/ch` is).

• All leading whitespaces for `RECIPE_NAME` will be trimmed.

• `INGREDIENT_QUANTITY__IN_RECIPE` must be an integer between 1 and 2,147,483,647 inclusive.
